...
const MODEL_PRESETS: Record<
  ModelSource,
  { repo: string; file: string; mmprojFile: string }
> = {
  bartowski: {
    repo: "bartowski/google_gemma-4-E4B-it-GGUF",
    file: "google_gemma-4-E4B-it-Q4_K_M.gguf",
    mmprojFile: "mmproj-google_gemma-4-E4B-it-bf16.gguf",
  },
  unsloth: {
...
const CONFIG = {
  ...
  MODEL_REPO: undefined as string | undefined,
  MODEL_FILE: undefined as string | undefined,
  MMPROJ_FILE: undefined as string | undefined,
  VISION_ENABLED: true,
};
...
function getModelConfig(): { repo: string; file: string; mmprojFile: string } {
  const preset = MODEL_PRESETS[CONFIG.MODEL_SOURCE];

  return {
    repo: CONFIG.MODEL_REPO ?? preset.repo,
    file: CONFIG.MODEL_FILE ?? preset.file,
    mmprojFile: CONFIG.MMPROJ_FILE ?? preset.mmprojFile,
  };
}

function getMmprojPath(): string {
  return path.join(getModelDir(), getModelConfig().mmprojFile);
}

function getModelId(modelFile: string): string {
  return modelFile.replace(/\.gguf$/i, "");
}

function getModelDisplayName(modelFile: string): string {
  return getModelId(modelFile);
}
...
async function downloadLlamaBinaries(): Promise<void> {
  ...
  if (!binConfig) {
    ...
    console.error(
      "Please build llama.cpp manually or use a supported platform.",
    );
    process.exit(1);
  }
...
function mmprojExists(): boolean {
  return fs.existsSync(getMmprojPath());
}
...
async function downloadModel(): Promise<void> {
  ...
  try {
    ...
    const hfCommand =
      process.platform === "win32" ? "huggingface-cli.exe" : "huggingface-cli";

    try {
      execSync(
        `${hfCommand} download ${modelConfig.repo} ${modelConfig.file} --local-dir "${modelDir}"`,
        {
          stdio: "inherit",
        },
      );
...
async function downloadMmproj(): Promise<void> {
  const modelDir = getModelDir();
  const modelConfig = getModelConfig();
  ...
  console.log(
    `\n🖼️ Downloading Gemma 4 vision projector (~${CONFIG.MMPROJ_SIZE_GB}GB)...`,
  );
  console.log(`   Source: ${CONFIG.MODEL_SOURCE}`);
  console.log(`   Repository: ${modelConfig.repo}`);
  console.log(`   File: ${modelConfig.mmprojFile}`);
  console.log(`   Destination: ${mmprojPath}\n`);
  ...
  try {
    ...
    try {
      execSync(
        `${hfCommand} download ${modelConfig.repo} ${modelConfig.mmprojFile} --local-dir "${modelDir}"`,
        {
          stdio: "inherit",
        },
      );
      ...
      console.log("huggingface-cli not found, using curl...");
      const url = `https://huggingface.co/${modelConfig.repo}/resolve/main/${modelConfig.mmprojFile}`;
      downloadFile(url, mmprojPath);
    }

    console.log("\n✅ Vision projector downloaded successfully!\n");
  } catch (error) {
    console.error("\n❌ Failed to download vision projector.");
    console.error("Please download manually from:");
    console.error(`https://huggingface.co/${modelConfig.repo}/tree/main`);
    console.error(`And place "${modelConfig.mmprojFile}" in: ${modelDir}`);
    process.exit(1);
  }
}
...
function startLlamaServer(): ChildProcess {
  ...
  const args = [
    ...
    modelPath,
    "--port",
    CONFIG.LLAMA_PORT.toString(),
    "-c",
    CONFIG.CONTEXT_SIZE.toString(),
    "--jinja",
  ];
...
function ensureOpencodeConfig(): void {
  ...
  const modelConfig = getModelConfig();
  const modelId = getModelId(modelConfig.file);
  const modelName = getModelDisplayName(modelConfig.file);
      ...
      "gemma4-local": {
        ...
        models: {
          "gemma4-e4b": {
            ...
            attachment: CONFIG.VISION_ENABLED,
            modalities: {
              input: CONFIG.VISION_ENABLED ? ["text", "image", "pdf"] : ["text"],
              output: ["text"],
            },
            reasoning: false,
            temperature: true,
...
async function main(): Promise<void> {
  program
    ...
    .option(
      "--vision",
      "Enable Gemma 4 vision support and download/use mmproj if needed (default: enabled)",
    )
    .option(
      "--no-vision",
      "Disable Gemma 4 vision support and skip mmproj handling",
    )
    .option(
      "--model-source <source>",
      "Model source: bartowski or unsloth",
      CONFIG.MODEL_SOURCE,
    )
    .option(
      "--model-repo <repo>",
      "Override Hugging Face repo, e.g. bartowski/google_gemma-4-26B-A4B-it-GGUF",
    )
    .option(
      "--model-file <file>",
      "Override GGUF file name, e.g. google_gemma-4-26B-A4B-it-IQ2_XXS.gguf",
    )
    .option(
      "--mmproj-file <file>",
      "Override mmproj GGUF file name, e.g. mmproj-google_gemma-4-E4B-it-bf16.gguf",
    )
    .option(
      "--port <number>",
  ...
  if (options.modelSource) {
    if (
      options.modelSource !== "bartowski" &&
      options.modelSource !== "unsloth"
    ) {
  ...
  if (options.modelRepo) {
    CONFIG.MODEL_REPO = options.modelRepo;
  }

  if (options.modelFile) {
    CONFIG.MODEL_FILE = options.modelFile;
  }

  if (options.mmprojFile) {
    CONFIG.MMPROJ_FILE = options.mmprojFile;
  }

  CONFIG.VISION_ENABLED = options.vision ?? true;
  ...
  if (options.forceDownload || !modelExists()) {
    ...
    console.log(
      `✅ Model found: ${path.join(getModelDir(), getModelConfig().file)}`,
    );
  }

  if (CONFIG.VISION_ENABLED) {
    if (options.forceDownload || !mmprojExists()) {
      await downloadMmproj();
    } else {
      console.log(`✅ Vision projector found: ${getMmprojPath()}`);
    }
  }
